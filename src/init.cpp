...
ZCJoinSplit* pseventeenseventysixParams = NULL;
...
void Shutdown()
{
    ...
    RenameThread("seventeenseventysix-shutoff");
    ...
    pseventeenseventysixParams = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "seventeenseventysix.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "seventeenseventysixd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 8235, 18235));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 8234, 18234));
...
{
    RenameThread("seventeenseventysix-loadblk");
    // -reindex
    if (fReindex) {
        CImportingNow imp;
        int nFile = 0;
...
static void ZC_LoadParams()
{
    ...
    if (!(boost::filesystem::exists(pk_path) && boost::filesystem::exists(vk_path))) {
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("Cannot find the SeventeenSeventySix network parameters in the following directory:\n"
              "%s\n"
              "Please run 'seventeenseventysix-fetch-params' or './zcutil/fetch-params.sh' and then restart."),
                ZC_GetParamsDir()),
    ...
    pseventeenseventysixParams->loadVerifyingKey(vk_path.string());

    gettimeofday(&tv_end, 0);
    elapsed = float(tv_end.tv_sec-tv_start.tv_sec) + (tv_end.tv_usec-tv_start.tv_usec)/float(1000000);
    LogPrintf("Loaded verifying key in %fs seconds.\n", elapsed);

    pseventeenseventysixParams->setProvingKeyPath(pk_path.string());
}
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SeventeenSeventySix version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SeventeenSeventySix is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SeventeenSeventySix is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SeventeenSeventySix is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    // Initialize SeventeenSeventySix circuit parameters
    ZC_LoadParams();
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("SeventeenSeventySix was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild SeventeenSeventySix with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty() && GetBoolArg("-gen", false)) {
        return InitError(_("SeventeenSeventySix was not built with wallet support. Set -mineraddress, or rebuild SeventeenSeventySix with wallet support."));
    }
 #endif // !ENABLE_WALLET
