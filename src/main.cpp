...
#if defined(NDEBUG)
# error "SeventeenSeventySix cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "SeventeenSeventySix Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state,
                      libseventeenseventysix::ProofVerifier& verifier)
{
    ...
    if (!CheckTransactionWithoutProofVerification(tx, state)) {
        ...
        BOOST_FOREACH(const JSDescription &joinsplit, tx.vjoinsplit) {
            if (!joinsplit.Verify(*pseventeenseventysixParams, verifier, tx.joinSplitPubKey)) {
...
void ThreadScriptCheck() {
    RenameThread("seventeenseventysix-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    auto verifier = libseventeenseventysix::ProofVerifier::Strict();
    auto disabledVerifier = libseventeenseventysix::ProofVerifier::Disabled();
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // In SeventeenSeventySix this has been enforced since launch, except that the genesis
    // block didn't include the height in the coinbase (see SeventeenSeventySix protocol spec
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, bool fRequested, CDiskBlockPos* dbp)
{
    ...
    auto verifier = libseventeenseventysix::ProofVerifier::Disabled();
